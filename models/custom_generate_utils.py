...
def get_important_heads(token_attn, image_pos, threshold=0.2):
    """
    token_attn: 一般是 outputs.attentions（tuple/list，每层一个 [B, H, Q, K]）
    image_pos:  (start, end)，这里在区间 (start+1 : end) 上统计
    threshold:  判定“重要头”的注意力和阈值
    """
    # print(image_pos)
    # important_heads = []   # 存 (score, layer_idx, head_idx)
    selected_attns = []    # 存每个重要头在该区间上的注意力分布 [L]

    start = image_pos[0] + 1
    end = image_pos[1]

    for _, layer_attn in enumerate(token_attn):
        # layer_attn 形状: [batch_size, num_heads, seq_len, seq_len]
        # 只看 batch=0、最后一个 query token、图像区间的 key：
        # interval_attn: [num_heads, 区间长度]
        interval_attn = layer_attn[0, :, -1, start:end]

        # 在 key 区间上求和 -> [num_heads]
        interval_sum = interval_attn.sum(dim=-1)
        # print(interval_sum)

        # 遍历每个头
        for head_idx in range(interval_sum.size(0)):
            score = interval_sum[head_idx].item()

            # 判断是否超过阈值，超过就收集
            if score > threshold:
                # important_heads.append((score, layer_idx, head_idx))
                # 保存这个头在该区间上的注意力分布（对该头是 [L]）
                selected_attns.append(interval_attn[head_idx])

    # 如果没有任何头超过阈值，就返回空和 None
    if len(selected_attns) == 0:
        avg_attn = None
        # assert 1==0
    else:
        # 堆叠 -> [num_selected_heads, L]
        selected_attns = torch.stack(selected_attns, dim=0)
        # 在头维度上求平均，得到该区间每个 token 的平均注意力 -> [L]
        avg_attn = selected_attns.mean(dim=0)

    # 也可以顺便把重要头按分数从大到小排序
    # important_heads.sort(key=lambda x: x[0], reverse=True)
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            for _logits_processor in logits_processor:
                if hasattr(_logits_processor, "set_attention_scores"):
                    _logits_processor.set_attention_scores(attention_scores)
                    _logits_processor.set_hidden_states(last_token_hidden)
